...
import { POSTCARD_ENDPOINTS, TRANSLATION_ENDPOINTS, API_BASE_URL } from '../constants/urls';

export type PostcardStatus = 'writing' | 'pending' | 'processing' | 'sent' | 'failed';

export type SendingStatus = 'translating' | 'converting' | 'generating' | 'sending' | 'completed' | 'failed';
...
export const postcardsApi = {
  getList: async (status?: PostcardStatus): Promise<PostcardResponse[]> => {
    ...
    return apiClient.get<PostcardResponse[]>(`${POSTCARD_ENDPOINTS.LIST}${params}`, true);
  },
  ...
  create: async (templateId?: string): Promise<PostcardResponse> => {
    ...
    return apiClient.post<PostcardResponse>(`${POSTCARD_ENDPOINTS.CREATE}${params}`, undefined, true);
  },
  ...
  update: async (id: string, data: UpdatePostcardData): Promise<PostcardResponse> => {
    ...
    const response = await fetch(`${API_BASE_URL}${POSTCARD_ENDPOINTS.UPDATE(id)}`, {
      method: 'PATCH',
      headers: {
  ...
  send: async (id: string): Promise<PostcardResponse> => {
    return apiClient.post<PostcardResponse>(POSTCARD_ENDPOINTS.SEND(id), undefined, true);
  },
  ...
  translate: async (text: string): Promise<TranslationResponse> => {
    return apiClient.post<TranslationResponse>(TRANSLATION_ENDPOINTS.TRANSLATE_TO_JEJU, { text }, true);
  },
};
